...
void loop(){
  ...
  if(set_gyro_angles){                                                 //If the IMU is already started
   // angle_pitch = angle_pitch * 0.9998 + angle_zero * 0.0002;     //Correct the drift of the gyro pitch angle with the set zero value (create artificial controlled drift)
   // angle_roll = angle_roll * 0.9998 + angle_zero * 0.0002;        //Correct the drift of the gyro roll angle with the set zero value
  }
  else{                                                                //At first start
  ...
  angle_pitch_output = angle_pitch;//angle_pitch_output * 0.9 + angle_pitch * 0.1;   //Take 90% of the output pitch value and add 10% of the raw pitch value
  angle_roll_output = angle_roll;//angle_roll_output * 0.9 + angle_roll * 0.1;      //Take 90% of the output roll value and add 10% of the raw roll value
  
  //activate servo
  deploy_chute(angle_pitch_output, angle_roll_output);
   Serial.print(angle_pitch_output);//90 to adjust pith given values
